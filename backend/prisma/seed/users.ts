...
export async function seedUsers(prisma: PrismaClient): Promise<SeededUsers> {
  // Departments are normalized — employees reference departmentId (was free-text).
  ...
  const kurt = await prisma.employee.create({
    data: {
      ...
      status: 'ACTIVE', // root node — no supervisorId
    },
  })

  // ADMIN. Also a supervisor by org graph (leads Team Alpha) — "supervisor" is derived, not a stored role.
  ...
  // EMPLOYEE role; SUPERVISOR is derived from the org graph (she has direct reports below).
