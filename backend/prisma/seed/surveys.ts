...
export async function seedSurveys(prisma: PrismaClient, users: SeededUsers): Promise<SeededSurveys> {
  ...
  for (let i = 0; i < activeAll.length; i++) {
    ...
    const resp = await prisma.surveyResponse.create({ data: { occurrenceId: s1Occ.id, employeeId: emp.id, ...snap } })
  ...
  for (const emp of activeAll) {
    await prisma.surveyAudienceMember.create({ data: { occurrenceId: s1Occ.id, employeeId: emp.id } })
  }
  ...
  for (const emp of activeAll) {
    await prisma.surveyAudienceMember.create({ data: { occurrenceId: s2Past.id, employeeId: emp.id } })
  }
  ...
  for (const emp of paMembers.slice(0, 8)) {
    const snap = await snapshot(emp)
    const resp = await prisma.surveyResponse.create({ data: { occurrenceId: s3Occ.id, employeeId: emp.id, ...snap } })
  ...
  for (const emp of activeAll) {
    await prisma.surveyAudienceMember.create({ data: { occurrenceId: s4Occ.id, employeeId: emp.id } })
  }
  ...
  for (const emp of rpMembers) {
    const snap = await snapshot(emp)
    const resp = await prisma.surveyResponse.create({ data: { occurrenceId: s5Occ.id, employeeId: emp.id, ...snap } })
