...
export interface EvaluationNotificationEmailParams {
  firstName: string;
  lastName: string;
...
/**
 * Builds the HTML body for a new performance evaluation notification email.
 * Layout follows the standard invitation pattern: logo, greeting, CTA, support copy, footer.
 */
export function buildEvaluationNotificationEmailHtml(
  params: EvaluationNotificationEmailParams,
): string {
...
<head>
  ...
  <title>New performance evaluation on ${BRAND.appName}</title>
</head>
          ...
          <tr>
            <td style="padding:0 40px 24px;">
              ...
              <p style="margin:0 0 16px;font-size:16px;line-height:24px;color:${BRAND.textSecondary};">
                Your supervisor has sent you a <strong style="color:${BRAND.textPrimary};">new performance evaluation</strong> to review and acknowledge.
