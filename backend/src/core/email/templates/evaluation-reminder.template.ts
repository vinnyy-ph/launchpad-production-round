...
export interface EvaluationReminderEmailParams {
  firstName: string;
  lastName: string;
...
/**
 * Builds the HTML body for an evaluation-acknowledgement reminder email — sent on a daily
 * cadence to a reviewee with a still-pending evaluation. Same layout as the initial
 * notification: logo, greeting, CTA, support copy, footer.
 */
export function buildEvaluationReminderEmailHtml(
  params: EvaluationReminderEmailParams,
): string {
...
<head>
  ...
  <title>Reminder: acknowledge your evaluation on ${BRAND.appName}</title>
</head>
      ...
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background-color:${BRAND.bgCard};border-radius:16px;box-shadow:${BRAND.shadow};overflow:hidden;">
          ...
          <tr>
            <td style="padding:0 40px 24px;">
              ...
              <p style="margin:0 0 16px;font-size:16px;line-height:24px;color:${BRAND.textSecondary};">
                This is a reminder that you have a <strong style="color:${BRAND.textPrimary};">performance evaluation</strong> from your supervisor still <strong style="color:${BRAND.textPrimary};">awaiting your acknowledgement</strong>.
                Click the button below to view and acknowledge it.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 40px 32px;text-align:center;">
              <a href="${evaluationUrl}" style="display:inline-block;background-color:${BRAND.buttonBg};color:${BRAND.buttonText};font-size:16px;font-weight:600;line-height:24px;text-decoration:none;padding:12px 20px;border-radius:8px;">
                View evaluation
              </a>
            </td>
          </tr>
          <tr>
            <td style="padding:0 40px 24px;">
              <p style="margin:0 0 16px;font-size:16px;line-height:24px;color:${BRAND.textSecondary};">
                Please acknowledge it before the deadline. If you have any questions, reply to this email and it will reach your HR team.
              </p>
          ...
          <tr>
            <td style="padding:24px 40px 40px;border-top:1px solid ${BRAND.border};">
              ...
              <p style="margin:0;font-size:14px;line-height:20px;word-break:break-all;">
                <a href="${evaluationUrl}" style="color:${BRAND.link};text-decoration:underline;">${evaluationUrl}</a>
              </p>
            </td>
          </tr>
        </table>
