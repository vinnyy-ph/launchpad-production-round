...
export interface OnboardingCompleteEmailParams {
  firstName: string;
  lastName: string;
  appUrl: string;
}
...
/**
 * Builds the HTML body for the onboarding-complete email an employee receives
 * once HR approves everything and activates their account.
 * Layout mirrors the invitation template: logo, greeting, status banner, CTA,
 * support copy, footer.
 */
export function buildOnboardingCompleteEmailHtml(
  params: OnboardingCompleteEmailParams,
): string {
...
<head>
  ...
  <title>Your ${BRAND.appName} account is active</title>
</head>
          ...
          <tr>
            <td style="padding:0 40px 24px;">
              ...
              <p style="margin:0 0 16px;font-size:16px;line-height:24px;color:${BRAND.textSecondary};">
                Great news — HR has reviewed everything and your <strong style="color:${BRAND.textPrimary};">${BRAND.appName}</strong> account is now active.
                You're all set to sign in and get started with your team.
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 40px 24px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:${BRAND.successBg};border:1px solid ${BRAND.successBorder};border-radius:12px;">
                <tr>
                  <td style="padding:16px 18px;font-size:16px;line-height:24px;font-weight:600;color:${BRAND.successText};">
                    Your account is active — onboarding complete.
                  </td>
                </tr>
              </table>
            </td>
