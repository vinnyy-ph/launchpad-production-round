...
describe("Document reviews - admin authorization", () => {
  beforeEach(() => {
    resetDocumentReviewMocks();
  });

  it("returns 403 when an admin tries to list submissions", async () => {
  ...
  it("returns 403 when an admin tries to approve a submission", async () => {
    const response = await request(app)
  ...
  it("returns 403 when an admin tries to reject a submission", async () => {
    const response = await request(app)
