...
describe("Invitations - admin authorization", () => {
  beforeEach(() => {
    resetInvitationMocks();
  });

  it("returns 403 when an admin tries to send an invitation", async () => {
  ...
  it("returns 403 when an admin tries to view invitation status", async () => {
    const response = await request(app)
  ...
  it("returns 403 when an admin tries to resend an invitation", async () => {
    const response = await request(app)
