...
| Rate limiting        | 500 requests / min (global); production `/api/auth`: 20 / min (failed attempts only); development `/api/auth`: 500 / min; production `trust proxy` for Railway | `rate-limit.middleware.ts`, `app.ts`                                     |
